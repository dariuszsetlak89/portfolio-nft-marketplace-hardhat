    ...
    : describe("NftMarketplace Unit Tests", function () {
          ...
          describe("listItem", function () {
              it("reverts when try to list already listed item", async function () {
                  ...
                  await expect(nftMarketplace.listItem(cuteNft.address, TOKEN_ID, PRICE)).to.be.revertedWith(
                      alreadyListedError
                  );
              });
              ...
              it("reverts when listing price is 0", async function () {
                  ...
                  await expect(nftMarketplace.listItem(cuteNft.address, TOKEN_ID, listingPrice)).to.be.revertedWith(
          ...
          describe("buyItem", function () {
              ...
              it("reverts when owner try to buy NFT item", async function () {
                  await nftMarketplace.listItem(cuteNft.address, TOKEN_ID, PRICE);
                  nftMarketplace = nftMarketplaceContract.connect(deployer);
                  await expect(nftMarketplace.buyItem(cuteNft.address, TOKEN_ID, { value: PRICE })).to.be.revertedWith(
                      "IsOwner"
                  );
              });
          ...
          describe("cancelListing", function () {
              it("reverts when try to cencel not listed item", async function () {
                  const notListedError = `NotListed("${cuteNft.address}", ${TOKEN_ID})`;
                  await expect(nftMarketplace.cancelListing(cuteNft.address, TOKEN_ID)).to.be.revertedWith(
                      notListedError
                  );
              });
          ...
          describe("updateListing", function () {
              it("reverts when try to update not listed item", async function () {
                  ...
                  await expect(
                      nftMarketplace.updateListing(cuteNft.address, TOKEN_ID, updatedPrice)
                  ).to.be.revertedWith(notListedError);
              });
